...
@section('content') 
    ...
    <div class="x_panel">
        <div class="x_title">
        <h2><i class="fa fa-bell-o"></i> Reportes Contenedores <i class="fa fa-angle-right text-danger"></i><small> Lista</small></h2>
        @role('root|red-frio')
            @permission('create.catalogos')
                <a class="btn btn-default pull-right" href="{{ url('reporte-contenedor/create') }}" role="button">Nuevo reporte</a>
            @endpermission 
        @endrole
        <div class="clearfix"></div>
        </div>
        <div class="x_content">
             @include('reporte-contenedor.list')
        </div>
    </div>
    <!-- Modal delete -->
    ...
    {!! Form::open(['route' => ['catalogo.red-frio.contenedor-biologico.destroy', ':ITEM_ID'], 'method' => 'DELETE', 'id' => 'form-delete']) !!}
    {!! Form::close() !!}
@endsection
@section('my_scripts')
    <!-- Datatables -->
