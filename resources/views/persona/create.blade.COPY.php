                    ...
                    <div class="x_content" id="content-esquema">
                        ...
                        @foreach($vacunas_esquemas as $key=>$ve)   
                            <?php 
                                $key_plus = $key; 
                                $key_plus = $key_plus + 1; 
                                $i_actual = $ve->intervalo_inicio; 
                                if((count($vacunas_esquemas) - 1) == $key) {
                                    $i_siguiente = 'none';
                                } else {
                                    $i_siguiente = $vacunas_esquemas[$key_plus]->intervalo_inicio;
                                }
                                $col_md = 1; $plu_col_md = 0;
                                foreach ($vacunas_esquemas as $k => $v) {
                                    if($ve->intervalo_inicio==$v->intervalo_inicio)
                                        $plu_col_md++;
                                }
                            ...
                            @endif
                                <div class="animated flipInY col-lg-{{$col_md}} col-md-{{$col_md}} col-sm-{{$col_md}} col-xs-12"><br>
                                    <div class="tile-stats" style="color:white; margin:0px; padding:3px; border:solid 2px #{{$ve->vacuna->color_rgb}}; background-color:#{{$ve->vacuna->color_rgb}} !important;">
                                        <div class="row">
                                            <div class="col-md-12"> <span style="font-size:x-large;font-weight:bold;"> {{$ve->vacuna->clave}} <small> @if($ve->tipo_aplicacion==1) Única @endif @if($ve->tipo_aplicacion==2) 1a Dosis @endif @if($ve->tipo_aplicacion==3) 2a Dosis @endif @if($ve->tipo_aplicacion==4) 3a Dosis @endif @if($ve->tipo_aplicacion==5) 4a Dosis @endif @if($ve->tipo_aplicacion==6) Refuerzo @endif  </small> </span> <span style="font-size:large;" class="pull-right"> @if($ve->intervalo_inicio<=29) Nacimiento @else  @if(($ve->intervalo_inicio/30)<=23){{($ve->intervalo_inicio/30)}} Meses @else {{round((($ve->intervalo_inicio/30)/12))}} Años @endif @endif  </span></div>
                                        </div>
                                        <div class="row">
                                            <div class="bt-flabels__wrapper">
            ...
            $.get('../catalogo/esquema/'+esquema, {}, function(response, status){ // Consulta esquema
                if(response.data==null){
                    ...
                    $.each(vacunas_esquemas, function( key, ve ) {
                        var key_plus = key; 
                        key_plus = key_plus + 1; 
                        var i_actual = ve.intervalo_inicio; 
                        if((vacunas_esquemas.length - 1) == key) {
                            var i_siguiente = 'none';
                        } else {
                            var i_siguiente = vacunas_esquemas[key_plus].intervalo_inicio;
                        }
